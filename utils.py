...
def generate_data(data, targeted=True, random_and_least_likely = False, skip_wrong_label = True, start=0, ids = None,
    ...
    true_ids = []
    information = []
    target_candidate_pool = np.eye(data.attack_labels.shape[1])
    target_candidate_pool_remove_background_class = np.eye(data.attack_labels.shape[1] - 1)
        ...
        if targeted:
            ...
            if random_and_least_likely:
                ...
                if target_classes is None:
                    original_predict = np.squeeze(predictor(np.array([data.attack_data[start+i]])))
                    ...
                    true_label = np.argmax(data.attack_labels[start+i])
                    seq = []
                ...
                if imagenet:
                    ...
                    information.extend(data.attack_labels.shape[1] * ['random'])
                else:
                    seq = range(data.attack_labels.shape[1])
                    information.extend(data.attack_labels.shape[1] * ['seq'])

            is_correct = np.argmax(data.attack_labels[start+i]) == predicted_label
            print("[DATAGEN][L1] no = {}, true_id = {}, true_label = {}, predicted = {}, correct = {}, seq = {}, info = {}".format(total, start + i, 
                np.argmax(data.attack_labels[start+i]), predicted_label, is_correct, seq, [] if len(seq) == 0 else information[-len(seq):]))
            if is_correct:
                n_correct += 1
            for j in seq:
                # skip the original image label
                if (j == np.argmax(data.attack_labels[start+i])):
                    continue
                ...
                true_labels.append(data.attack_labels[start+i])
                if remove_background_class:
            ...
            if remove_background_class:
                ...
                print(np.argmax(data.attack_labels[start+i]))
                print(np.argmax(data.attack_labels[start+i][1:1001]))
                targets.append(data.attack_labels[start+i][1:1001])
            else:
